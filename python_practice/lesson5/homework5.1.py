...
def find_cars(car_data, search_criteria):
    ...
    filtered_cars = [
        ...
        if details[1] >= min_year
        and details[2] >= min_engine_volume
        and details[4] <= max_price
    ]

    sorted_cars = sorted(filtered_cars, key=lambda car: car[5])

    return sorted_cars[:5]

result = find_cars(car_data, search_criteria)

for brand, color, year, engine_volume, car_type, price in result:
    print(f"{brand}: {color}, {year}, {engine_volume}L, {car_type}, ${price}")